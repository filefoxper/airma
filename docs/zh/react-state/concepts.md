...
非稳定实例的缺陷在于它不符合稳定闭包理念，可能导致实例属性值在 `React.Strict` 严格模式状态下（如 useEffect 依赖中）具有不稳定性。当然它实时获取实例属性值的能力是非常有用的。所以，这里推荐在与渲染及副作用依赖无关的回调函数中使用它，这可以为你省去大量闭包的麻烦。

无论渲染实例是否稳定，在[刷新](/zh/react-state/concepts?id=刷新)过程中，模型函数的内部状态值是始终可靠的。

## 刷新

`刷新`是指来自`实例`的`行为方法`被调用后，其返回值被 `useModel` API 作为新状态参数再次用于调用模型函数，并更新 `实例` 的自动化过程。
...
在 `useModel` 加载，以及点击调用 increase 行为方法的过程中，我们的探针 `console.log` 均为我们发来了关键数据。
...
```
通过以上命令式的打印过程，我们可以更清楚地了解有关刷新的过程。

## 链接

链接是 `@airma/react-state` 的基本工作单元对象，`useModel` API 就是通过链接对象来存储`模型`、`实例`、`状态`等信息的。除了存储工作单元信息，链接还负责协调`实例`的刷新，渲染等自动化过程。

在了解了 `@airma/react-state` 的概念和工作原理后，让我们进入最让人振奋的 [引导](/zh/react-state/guides.md) 章节，看看关于该工具的高级功能（跨组件状态同步），以及如何在实际工作中用好它。
