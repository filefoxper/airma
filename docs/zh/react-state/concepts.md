...
非稳定实例的唯一缺陷就是它不符合稳定闭包理念，在导致了在 `React.Strict` 严格模式的模型状态下（如 useEffect 依赖中）非常不稳定，时常出现类似 bug 的现象，但它实时获取属性值的能力在各个回调函数中却异常有用。所以，这里推荐在与渲染及副作用依赖无关的回调函数中使用它，这可以为你省去大量闭包的麻烦。

当然，无论实例是否稳定，在行为方法的调用及其触发的模型[刷新](/zh/react-state/concepts?id=刷新)过程中，状态值始终会保持最新状态。即，在模型函数范围内，`state` 状态始终是安全的。

## 刷新

`刷新`是指`行为方法`调用后，其返回值被 `useModel` API 作为新状态参数用于再次调用模型函数，并更新 `实例` 的自动过程。
...
我们通过调用一次来自 `实例` 的 `行为方法` 即可触发一次 `刷新` 过程。这里记录了 `counter` 模型通过 `increase` 触发刷新的全过程。

```
[人工] increase 返回 count + 1，记为 next，值为 0；
