...
本地状态管理需要为 useModel 提供一个[模型](concepts?id=模型)函数及模型对应的默认状态。
...
1. 其中本地管理使用的模型函数，需要换成[键](concepts?id=键)。
...
自 `v18.4.0` 开始 `@airma/react-state` 新增了 [useSignal](api?id=usesignal) API，用于提升渲染性能，并监听模型实例变更。
