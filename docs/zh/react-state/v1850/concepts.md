...
键是生成库的模板，也是访问库的钥匙。所谓的[动态库](index?id=为什么要支持-reactcontext-库管理模式？)就是在组件中建立的库，因为这些库难以通过模块获取，所以需要通过键来访问。
...
[provide](api?id=provide) 高阶组件 或 [Provider](api?id=provider) 组件需要使用键来创建本地库。
...
[useModel](api?id=usemodel) 或 [useSelector](api?id=useselector) 通过键连接已创建的本地库。
