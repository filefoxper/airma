...
* modelFn/modelKey/modelStore - [模型](concepts?id=模型)函数或[键](concepts?id=键)或[库](concepts?id=库)) 。
* initialState - 初始化状态值，当 modelLike 为键或库时为可选项。

返回：

模型实例对象（如使用produce定制模拟实例，则为模拟实例）
...
该 API 有助于将模型复用至受控模式，`useControlledModel` 不维护内部状态，完全受控于传入的外部状态值 state，可参考引导中关于[useControlledModel](guides?id=usecontrolledmodel)这部分。

返回：

完全受控的模型实例对象（如使用produce定制模拟实例，则为模拟实例）
...
* modelKey/modelStore - 模型[键](concepts?id=键)或[库](concepts?id=库) 。
* selector - 数据选择函数，用于选取重组库存实例（如使用produce定制模拟实例，则为模拟实例）数据。
* equality - 可选等价对比函数，用于自定义判断状态变更前后的选取值是否等价。默认以 `===` 作为对比方案。若等价，即便库状态发生了变更，也不重新渲染组件。

解释：

若 `useSelector` 选取值在状态更新前后等价，则不触发组件再渲染，否则更新选取值并触发组件渲染。对比标准 `equality`。
...
* modelLike - [模型](concepts?id=模型)函数或[键](concepts?id=键)或库(通过model().createStore()建立的静态库) 。
...
关于如何使用 useSignal，及 signal 函数的 effect 和 watch 方法，请参考引导中的[高性能渲染](guides?id=高性能渲染)章节。
...
* value - [键](concepts?id=键)集合，可以是单个键，也可以是键的数组或对象集合形式。
...
* storeCreators - [键](concepts?id=键)集合，可以是单个键，也可以是键的数组或对象集合形态。
* component - React 组件
...
* modelFn - [模型](concepts?id=模型)函数
* initialState - 可选初始化状态
...
通常用于 [useSelector](api?id=useselector) 的 `equalFn` 参数，以优化渲染性能。
...
用法[参考](guides?id=model)

静态方法

* createField - 创建实例字段，用法[参考](guides?id=实例字段)
