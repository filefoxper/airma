...
本节主要介绍 **@airma/react-state** 中各种常用 API。

## useModel

API useModel 可用于访问本地库、静态库、动态库的实例对象。

### 本地状态管理

本地状态管理需要为 useModel 提供一个[模型](concepts?id=模型)函数及模型对应的默认状态，useModel 会自行创建一个本地库来管理状态。
...
可以把它当作便写版的 React.useReducer。

### 动态库状态管理

用于管理动态库实例的 useModel 与管理本地库实例的 useModel 略有不同。

1. 当访问本地模型实例时，useModel 会在本地创建一个本地库；而访问动态库的 useModel 只负责访问。
2. 创建和访问本地库使用的是模型函数；访问动态库需要使用[键](concepts?id=键)。
3. 访问本地库，必须提供初始化状态；而访问动态库时，若键有预设默认状态，或其他同键 useModel 已做初始化处理，则可不提供默认状态。
...
useSelector API 可用于选取并重组模型实例的属性值，通过对比状态变更前后选取值是否等价，来决定是否需要重新渲染。可用于提升渲染性能。

```ts
const xxx = useSelector(modelKey, (instance)=>instance.xxx);
```

可通过第三个参数 equalFn 可自定义如何判断重组值是否相等。推荐使用 [shallowEqual](api?id=shallowequal) API。
...
API [provide](api?id=provide) 用于提供创建和使用动态库所需的 [Provider](api?id=provider) 外包装组件环境。
...
API [createKey](api?id=createkey) 可以为**模型**生成[键](concepts?id=键)，键可用于生成本地库，同时作为连接本地库的通道。
...
关键渲染数据完全依赖 props 提供和回传的 React 组件被称为受控组件。受控组件具有非常良好的复用性，状态控制权交由使用者掌控。在受控组件中可使用 [useControlledModel](api?id=usecontrolledmodel) 可适配通用模型来完成较复杂的受控交互任务。
...
模型声明 API [model](api?id=model) 整合了常用的模型 API，提供了一套流调用的使用方式。
...
// provide(toggleGlobalStore).to(function Component(){
...
因为静态库本身含有键，故可以被 provide 作为键来产生动态库。但并不推荐这种动态库创建方式。
...
const countKey = model((count: number)=>({
    ...
    increase:()=> count + 1,
    decrease:()=> count - 1,
    add:(addition: number)=> count + addition
    // 通过 produce 方法可加工模拟实例对象
}).produce((getInstance)=>{
    const instance = getInstance();
    // model.createField 在默认选择器中也是生效的
    const info = model.createField(()=>{
        return {isNegative: instance.count<0};
    }, [instance.count]);
    return {
        ...instance,
        info,
        async increaseBySetting(){
            ...
            // getInstance 可获取最新的实例对象
            return getInstance().add(setting);
        }
    }
}).createKey(0);

const App = provide(countKey).to(()=>{
    // 在 produce 加工出模拟实例后，useModel 只能返回模拟实例
    const {
        // 模拟实例的方法 increaseBySetting 和行为方法一样，也是持续不变的
        increaseBySetting, 
        // model.createField 的缓存功能在模拟实例中也能生效
        info
    } = countKey.useModel();
    ......
})
...
虽然通过 produce API 可以比较理想地模拟异步行为，并做到状态同步和异步逻辑分离，但依然推荐使用类似 useQuery/useMutation 这类专门为异步状态管理设计的工具。

关于如何通过 model 使用 useSignal 来优化渲染性能，请参考 [高性能渲染](guides?id=高性能渲染) 中的内容。
...
model.createField 方法可用于创建一个实例字段，通过对该方法添加缓存依赖项，可使该字段对外具备缓存能力。只有当该字段取自库实例时才具备缓存特性，可通过字段的 get 方法获取字段值。
...
useSignal API 返回一个实例生成函数，调用该函数可获取当前最新的实例对象。在组件渲染过程中 useSignal 会统计实例对象各字段的使用情况，当这些字段发生变更时，useSignal 才会重新渲染组件，否则不主动渲染组件。
...
注意：signal.useEffect 和 signal.useWatch 只能订阅库存实例的属性变更和行为方法，并不能对模拟实例其效。
...
const counting = model((state:number)=>({
    ...
    const {count, decrease} = getInstance();
    return {
        count,
        decrease,
        async increaseBySetting(){
