...
用于创建查询[会话](concepts?id=会话)的 React Hook API。只采纳最新执行返回的会话结果，默认情况下，通过组件加载、依赖更新、人工调用三种方式均可触发。

无 config 入参的 useQuery 与 useSession 的功能类似，当该 useQuery 被人工触发时，它会先查找是否存在会话键或会话库相同，且具备 config 参数的其他 useQuery，若存在，则驱动其工作，若不存在或无法驱动其他 useQuery 工作，则自己工作。
...
* **promiseCallbackOrSessionKeyOrSessionStore** - 异步函数，承载异步函数的[会话键](concepts?id=键)，或承载异步函数的[会话库](concepts?id=库)。
* **variablesOrConfig** - 异步函数的参数列表或[会话配置](concepts?id=会话配置)。

### 返回

元组 `[sessionState, trigger, execute]`

* **sessionState** - [会话状态](concepts?id=会话状态)
* **trigger** - [会话触发器](concepts?id=触发和执行)
* **execute** - [会话执行器](concepts?id=触发和执行)

## useMutation

用于创建修改操作[会话](concepts?id=会话)的 React Hook API。在手工触发情况下阻塞运行，默认情况下，只能通过人工调用触发。

无 config 入参的 useMutation 与 useSession 的功能类似，当该 useMutation 被人工触发时，它会先查找是否存在会话键或会话库相同，且具备 config 参数的其他 useMutation，若存在，则驱动其工作，若不存在或无法驱动其他 useMutation 工作，则自己工作。
...
可将异步函数包装成[会话键](concepts?id=键)的函数方法。
...
可将异步函数包装成[库](concepts?id=库)的函数方法。
...
```ts
Provider props:{
    value: <key> or <keys array> or <keys object>,
    children?: ReactNode
}
```
...
用于订阅库会话状态变更的 React hook API，也可用于人工触发同键或同库会话执行。

```ts
function useSession(sessionKeyOrStore):[sessionState, trigger, execute]
```

### 参数

* sessionKeyOrStore - 会话键或会话库。
...
* **sessionState** - [会话状态](concepts?id=会话状态)
* **trigger** - [会话触发器](concepts?id=触发和执行)，不能传参，只能触发。
* **execute** - [会话执行器](concepts?id=触发和执行)，允许传参触发。
...
```ts
function useLoadedSession(sessionKeyOrStore):[sessionState, trigger, execute]
```
...
* **sessionState** - [会话状态](concepts?id=会话状态)，sessionState.data 类型与异步函数返回数据类型相同，且 sessionState.loaded 为 true。
* **trigger** - [会话触发器](concepts?id=触发和执行)
* **execute** - [会话执行器](concepts?id=触发和执行)，允许传参触发。

## Strategy

**@airma/react-effect** 提供的常用[策略](concepts?id=策略)集合。
...
当策略链中没有 Strategy.response.failure 时 Strategy.failure 策略会暂停前置 Strategy.failure 策略，若回调函数抛出异常，则按至后往前的方向将异常提交给前置 Strategy.failure 策略的回调函数处理，否则结束异常处理。

若策略链中存在 Strategy.response.failure, 则等待 Strategy.response.failure 提交异常，直至 Strategy.response.failure 处理结束。
...
会话执行响应策略。用于监听会话执行，并在会话执行完毕，并更新完会话状态后调用回调函数。
...
会话执行成功响应策略。用于监听会话执行，并在会话执行成功，且会话状态更新完成后调用回调函数。
...
会话执行失败响应策略。用于监听会话执行，并在会话执行失败，且会话状态更新完成后调用回调函数。

**当前策略在会话状态更新完毕的副作用（useEffect）中运行回调函数。**

Strategy.response.failure 会暂停所有 Strategy.failure 和前置 Strategy.response.failure 错误处理策略，在其回调函数抛出的异常时重新激活它们，并按至后往前的方向把异常递交给更高层的 Strategy.response.failure 或 Strategy.failure 处理。
...
用法可参考引导章节中关于[全局策略配置](guides?id=strategy)和[ConfigProvider](guides?id=configprovider)中的内容。

## useLazyComponent

监听多个会话是否已执行过或已加载，异步加载组件，返回一个 React.lazy 包装组件。如被监听会话均已加载，则 React.lazy 正常渲染；如被监听会话均已执行且部分会话出错，则 React.lazy 组件接收到一个名为 'error'，值类型为[会话状态](concepts?id=会话状态)类型的 Props 属性。 
...
React hook，用于监听会话执行，并在执行完毕后调用回调函数。回调函数的确切执行时机发生在会话执行完毕，且更新会话状态后产生的副作用中（useEffect）。
