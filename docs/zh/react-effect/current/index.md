...
每个异步状态管理单元都被称为[会话](concepts?id=会话) (session)，无论 useQuery 还是 useMutation 都是创建会话的工具。通过 [session](api?id=session) 会话声明 API 来使用 useQuery 和 useMutation 更方便，更准确。
...
useQuery/useMutation 直接使用异步函数创建的是一个本地会话。通过 session.createKey() 的方式可以创建一个动态会话键，动态会话键可用于创建动态会话库，并可作为订阅该库的钥匙同步库中的会话状态。
...
// 动态会话键会在 provide 高阶组件元素化过程中建立内部库，
// 子组件可以通过会话键连接该库
...
动态会话键作为生成库的模版提供给 provide 高阶组件，高阶组件在组件元素化过程中利用键创造库。因此每个使用了会话键的组件可以同时生成多个持有不同会话库的元素，这些库互不干扰，即为动态库。
...
通过 useSession API 获取的[会话状态](concepts?id=会话状态)数据类型（data）可能是 undefined。如确认使用的会话已成功加载（会话已执行成功过，或已设置了默认数据），可使用 useLoadedSession API 来代替 useSession 工作，这时获取的**会话状态数据**类型完全与异步函数返回的 Promise resolve 类型相同。
...
[Strategy](api=strategy) API 提供了丰富的常用策略。单独或组合使用这些[策略](concepts?id=策略)，可修改会话的运行效果。
...
**@airma/react-effect** 用于管理异步状态，它依赖了 [@airma/react-state](/zh/react-state/index) 工具包，并借助了如键、动态库等概念，因此在 API 上与 **@airma/react-state** 有很多相似之处。如果需要同时使用这两种不同的状态管理工具，可选用 [@airma/react-hooks](/zh/react-hooks/index)，它整合了两个工具的常用 API，更便于使用。
