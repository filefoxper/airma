...
* 移除部分被废弃的 session API：
  * session().createStore().static(), 
  * session().createStore().with(), 
  * session().createStore().provide(), 
  * session().createStore().provideTo(), 
  * session().createStore().Provider()
* 移除部分被废弃的 Strategy API:
  * Strategy.error
  * Strategy.response.error
* 修改 Strategy.failure 和 Strategy.response.failure 的特性：
  * Strategy.failure 和 Strategy.response.failure 可抛出异常给策略链前方的 Strategy.failure 处理，若不抛出异常，则不再运行前置 Strategy.failure。
* 添加 *createSessionStore* API 创建静态会话库，因作用和 session().createStore() 一致，可选择使用。
* 添加了会话执行身份 token 机制控制 sessionState.isFetching 和 sessionState.round。
