...
API [useQuery](api?id=usequery) 用于建立异步查询会话。查询会话基本策略始终以最新一次执行产生的数据为会话状态数据。查询会话默认以全量触发模式运行（即：支持加载、更新、人工触发模式）。

查询会话可接收一个异步函数作为最终执行函数，也可以使用会话[键](concepts?id=键)执行异步函数。
...
使用 useQuery 管理库存会话，常常需要与 [useSession](guides?id=usession) API 配合使用，相关内容介绍在引导的
[provide](guides?id=provide) 和 [useSession](guides?id=usession) 小节中。

## useMutation

API [useMutation](guides?id=usemutation) 用于管理修改会话。默认情况下只支持原子性的人工触发方式。
...
库的使用可以让 React 状态管理变得更加简单易懂。`@airma/react-effect` 支持创建**动态**和**静态**两种不同形态的库。 [Provider](api?id=provider) 组件通过持有[键](concepts?id=键)可动态创建一个维护在 Provider 内部的本地库，useQuery/useMutation/useSession 可通过键订阅库的状态变更，也可通过执行触发更新库的状态。
...
API [useSession](api?id=usesession) 可通过会话[键](concepts?id=键)订阅本地库中的会话状态，也可通过 trigger 和 execute 方法人工触发 useQuery/useMutation 执行会话。
