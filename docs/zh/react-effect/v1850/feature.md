...
`@airma/react-effect` 完全使用了 `@airma/react-state` 的 [Provider 系统](/zh/react-state/guides?id=createkey-与-provide)，所以 provide，Provider，store.with(...stores) API是完全互通的。而 API createSessionKey 创建的会话[键](concepts?id=键)实际上就是一种特殊的模型[键](/zh/react-state/concepts?id=键)。
