...
用于创建查询操作[会话](concepts?id=会话)的 React Hook API。只采纳最新执行返回的会话结果，默认情况下，通过加载、依赖更新、人工调用均可触发。
...
* **promiseCallbackOrSessionKey** - 异步函数，或承载异步函数的[会话键](concepts?id=键)。
...
* sessionKey - 会话键，可通过 [createSessionKey](api?id=createsessionkey) API 创建。
...
* **execute** - [会话执行器](concepts?id=触发和执行)，允许传参触发。**至  v18.3.2 开始支持**。
