...
对一个查询来说，永远接收最新一次触发的请求结果为会话状态是非常有必要的。所以 `useQuery` 通过使用一个不透出的内部策略实现了这一目标，也就是说无论用不用策略，你所使用的 `useQuery` 渲染的永远是最新请求的会话结果。

比如翻页查询功能：

假设有一个没有设限的翻页器，如果我们快速连续翻第2，第3页，这时每个请求都已经发向服务端，我们并不知道哪页数据会先被服务端返回给我们。如果，`useQuery` 没有做最新数据保护，那么可能出现第3页数据先返回并渲染在我们的页面中，之后姗姗来迟的第2页数据将最新请求的第三页数据覆盖掉，从而导致了页码与数据不对应的问题。所以本特征对 `useQuery` 来说是必要的，`useQuery` 通过对迟到请求的会话结果标记 `abandon` 废弃信息拦截了迟到数据的渲染工作。
