...
This section tells how to use these APIs: [useQuery](guides?id=usequery), [useMutation](guides?id=usemutation), [useSession](guides?id=usessesion), [provide](guides?id=provide), [session](guides?id=session) and [Strategy](guides?id=strategy).

## useQuery

API useQuery is used for managing a query session state. It can run with a promise callback or a session [key](concepts?id=key), event a session store. It always takes a latest  execution result as [session state](concepts?id=session-state). Consider it as **useEffect** is helpful for understanding when it works automatically.
...
API useMutation is used for managing a mutation session state. It can run with a promise callback or a store session [key](concepts?id=key). The execution is atomical, when it is triggered manually. By default, it only can be triggered manually.
...
The [Provider](api?id=provider) component creates store from keys, so creating session [key](concepts?id=key) is the first step.
...
API [createSessionKey](api?id=createsessionkey) wraps promise callback to be session key.

#### Use session keys to create store

API [provide](api?id=provide) is a HOC (Higher order component), it creates a [Provider](api?id=provider) wrapper (HOC too).
...
Use [Provider](api?id=provider) component directly to create store.
...
API [useSession](api?id=usesession) only subscribes session state change of store, it can not execute promise callback itself, but can drive useQuery/useMutation to work. 
...
More [examples](index?id=static-store-state-management).

## create store

Api [createSessionStore](api?id=createsessionstore) can help for creating a static session store just like **session(xxx, xx).createStore()**.

```ts
// session.ts
import {createSessionStore} from '@airma/react-effect';

// create a session store for query with promise callback
const queryStore = createSessionStore(fetchUsers, 'query');
```

```ts
// usage.tsx
import {queryStore, saveKey} from './session';
import {
    provide,
    useQuery,
    useSession
...
const Child1 = ()=>{
    const [
        ...
        triggerQuery,
        executeQuery
    ] = useSession(queryStore);
    return ......;
}

const Child2 = ()=>{
    const q = useMemo(()=>({name:'',username:''}),[]);
    useQuery(queryStore, [q]);
    return ......;
}

// just like session(xxx, xx).createStore(),
// the static store needs no provide
const Component = (props:Props)=>{
    useQuery(queryStore, [props.query]);
    return (
        <>
            <Child1 />
            <Child2 />
...
The [strategies](concepts?id=strategy) are often chained and used on useQuery/useMutation.

Set common strategies on [ConfigProvider](api?id=configprovider), then every useQuery/useMutation can use these chained strategies automatically.
...
const globalConfig: GlobalConfig = {
    ...
    strategy: (
        ...
        // failure strategy should be top,
    ...
    useQuery(promiseCallback, {
        ...
        strategy:[
            Strategy.response.failure((err)=>{
                // it is allowed to process some conditions about error, and throw others to the default Strategy.failure process.
                if(err.code === 'xxx'){
                    processError(err);
                    return;
                }
...
A [strategy](concepts?id=strategy) function should accepts a runtime parameter, and returns a promise which resolves a session state like data.

If the resolved data.**abandon** is **true**, this data will be abandoned, it can not be a session state. That's why the **'abandon'** field in [session state](concepts?id=session-state) is always **false**.
...
const App = ()=>{
    // It works with common strategies: 
