...
* roll `useResponse` and `useResponse.*` features back. [rollback reason](guides?id=useresponse)
...
* remove some deprecated session API: 
...
* remove some deprecated Strategy API:
  * Strategy.error
  * Strategy.response.error
* make Strategy.failure and Strategy.response.failure more reasonable. Support throw exception to previous Strategy.failure.
