...
1. [session](concepts?id=session)
2. [key](concepts?id=key)
3. [strategy](concepts?id=strategy)

## Session

Session is a promise callback manage mission. It contains a state and a promise execute callback. API [useQuery](api?id=usequery) and [useMutation](api?id=usemutation) can build a local session or subscribe store session, API [useSession](api?id=usesession) can subscribe store session, API [session](api?id=session) can wrap a promise callback to be a session API collection.
...
The session state is always exsit, and it is changed by session executing. 
...
const {
    ...
    data,
    // The latest execution parameters.
    variables,
    ...
    // If the latest execution is successful, it is undefined
    error,
    ......
} = sessionState;
```
...
* **error** - The latest execution failed result. It should be undefined before session has been executed, or when the execution is successful. 
...
* **abandon** - It should be *false* in session state. In [stratey](concepts?id=strategy) executing process, setting it *true* can prevent invalid execution state to be adopted.
* **vistied** - It is similar to *loaded*, but can be changed by strategies (ex. [Strategy.cache](api?id=strategy-cache)).
...
* **online** - It should be *true* in a workable session. When the session holder point is unmounted, it will be set to *false*. Ex. The local usage about **useQuery/useMutation** and store usage about **provideTo component** are session holder point. Use [Strategy.validate](api?id=example) to access this field can prevent offline session executions.

### Session config

Session config tells [useQuery](api?id=usequery)/[useMutation](api?id=usemutation) how to execute promise callback.

The simplest config is a parameters array for promise callback.

```ts
// By default, useQuery start executing with parameters,
// when it is mounted, or some of parameters changes, or is triggered manually. 
useQuery(promiseCallback, [param1, param2, ...]);

// By default, useMuation only can be triggered manually to execute.
const [
    ,
    trigger,
    execute
...
All the config options are optional.
...
* **defaultData** - The default data for session. If it is set, the *sessionState.data* has a default data at begining, and *sessionState.loaded* is always *true*.
* **strategy** - The executing features for session. It can be a single [strategy](concepts?id=strategy), an strategy array or an [config object](concepts?id=strategy-config).
...
The **strategy** chains in working session is composed by customized strategies, a common strategies wrapper function and a default strategy for useQuery or useMutation.
...
* useMutation - blocking strategy, it ensures that when useMutation is executing manually, no other execution can be allowed to happen.
...
If a session supports manual execution by its **triggerOn** setting, it can be triggered by calling **trigger** or **execute** method.

The **trigger** method accepts no parameters, it just make session execute with config varibles.

The **execute** method needs parameters of promise callback for executing.
...
The manual execute methods allow to set a payload data for current execution, and this payload data will appear in session state fields, when the execution is finished.
...
Key is a special function for creating and subscribing store in [Provider](api?id=provider).  API [createSessionKey](api?id=createsessionkey) wraps a promise callback to be key. It is a special [model key](/react-state/concepts?id=key) from *@airma/react-state* concepts.
...
const App = provide(userQueryKey)(()=>{
    ...
    const [
        state, 
        // With session key, 
        // useQuery can subscribe session state from store.
...
* **getSessionToken** - A callback to get current session token.
* **varaibles** - Parameters for current execution.
* **triggerType** - Current execution trigger way, *'mount' | 'update' | 'manual'*.
* **abandon** - A callback to mark *abandon* property to target execution state.
...
* [useQuery](api?id=usequery) - The default strategy ensures that useQuery always sets a latest execution data to session state. 
* [useMutation](api?id=usemutation) - The default strategy ensures that useMutation always execute atomically when it is triggered manually.
...
Take next section [guides](guides).