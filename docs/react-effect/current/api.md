...
It is a query [session](concepts?id=session) API. By the default, it can be triggered when it is mounted; the dependency parameters change; or a manual trigger/execute is called. 

A no config useQuery works more like a useSession, it always looks for another useQuery with same session key/store and config to work for it. If the perfect substitute is not exist, it works manually. 
...
```ts
function useQuery(
  promiseCallbackOrSessionKeyOrSessionStore, 
  variablesOrConfig?
):[
  sessionState, 
  trigger, 
...
* promiseCallbackOrSessionKeyOrSessionStore - It can be a promise callback, a session [key](concepts?id=key) or a session store.
* variablesOrConfig - It is optional, if there is no variables, useQuery works in manual mode. It can be an parameter array for callback, or a [session config](concepts?id=session-config).

### Returns

* sessionState - [session state](concepts?id=session-state).
* trigger - A callback to trigger useQuery work manually. It need no parameter, useQuery uses option `variables` as parameters for execution.
* execute - A callback to execute useQuery manually. It need full parameters for execution.

## useMutation

It is a mutation [session](concepts?id=session) API. By the default, it can be triggered when  a manual trigger/execute is called. 

A no config useMutation works more like a useSession, it always looks for another useMutation with same session key and config to work for it. If the perfect substitute is not exist, it works manually. 

So, a no config useQuery looks more like a useSession. The only different is that the no config useQuery still works if no substitute session usage is found.

```ts
function useMutation(
  promiseCallbackOrSessionKeyOrStore, 
  variablesOrConfig?
):[
  sessionState, 
  trigger, 
...
* promiseCallbackOrSessionKeyOrStore - It can be a promise callback or a session [key](concepts?id=key), event a session store.
* variablesOrConfig - It is optional, if there is no variables, useMutation works in manual mode. It can be an parameter array for callback, or a [session config](concepts?id=session-config).
...
It wraps a promise callback to be a session [key](concepts?id=key) for creating store.
...
* SessionKey - It is a session [key](concepts?id=key).
...
* SessionStore - It is a static session store.

## Provider

It is a React Context Provider component. It creates store from [keys](concepts?id=key). It can be replaced by [Provider](/react-state/api?id=provider) in @airma/react-state package.
...
* value - [Session keys](concepts?id=key), [model keys](/react-state/concepts?id=key) or store.
...
function provide(keys){
    const hoc = function connect(Component){
    ...
    hoc.to = function toConnect(Component){
      return function HocComponent(componentProps){
            return (
                <Provider storeCreators={keys}>
                  <Component {...componentProps}/>
...
* sessionKey - A session [key](concepts?id=key).
...
* execute - A callback to execute useQuery/useMutation work manually. It need parameters for the execute function.

## useLoadedSession

It is a special useSession when the usage is ensure that the [session](concepts?id=session) has been loaded.
...
* sessionState - A loaded [session state](concepts?id=session-state).
* trigger - A callback to trigger useQuery/useMutation work manually. It need no parameter, useQuery/useMutation uses setted `variables` as parameters for execution.
* execute - A callback to execute useQuery/useMutation work manually. It need parameters for the execute function.

## Strategy

It is a set for often used [strategies](concepts?id=strategy).
...
Used for opening SWR query mode. When the query key (default `JSON.stringify(variables)`) can be found in cache, the cache data can be picked out as [session state](concepts?id=session-state).data immediately. In that case, if the **staleTime** is set, the execution skips.
...
It disables the previous failure strategies when it has processed all exceptions, and throws nothing out. When the process throw a exception, the system passes this exception to a next Strategy.failure process.

**Note, if there is some Strategy.response.failure, all the Strategy.failure will be disabled, and wait for some Strategy.response.failure process throw exception to active them.**
...
const App = ()=>{
  useQuery(callback,{
    ...
    strategy:[
      Strategy.failure((e)=>{
        // only process 403 exception
        if(e.status === 403){
          console.log('403');
          return;
        }
        // throw others to the global Strategy.failure
        throw e;
      })
    ]
  })
...
It is used for reducing render frequency. If the execution data equals with [session state](concepts?id=session-state) data, it uses the old session state data as a new session state data.
...
**It works in react.useEffect after a execution result is set as a new session state.**

This strategy can stop all Strategy.failure temporarily, and active a previous Strategy.failure or Strategy.response.failure when a exception is throw out.
...
A React.lazy component with **props.error** field, if all session states are loaded successfully, the component Suspense wrapper renders the component without **props.error: undefined**, unless the error should be like a failed executed [session state](concepts?id=session-state).
...
[Examples](index?id=session) before.
