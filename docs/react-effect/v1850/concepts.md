...
Session is a promise callback mission. It contains a state and a promise execute callback. API [useQuery](api?id=usequery) and [useMutation](api?id=usemutation) can build a session, API [useSession](api?id=usesession) can use a store session, API [session](api?id=session) can wrap a promise callback to be a session API collection.
...
* **abandon** - It should be `false` in session state. In [stratey](concepts?id=strategy) executing process, setting it `true` prevents invalid session state to be adopted.
...
Session config can be setted to tell [useQuery](api?id=usequery)/[useMutation](api?id=usemutation) when and how to execute promise callback.
