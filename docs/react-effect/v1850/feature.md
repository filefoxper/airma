...
API useQuery/useMutation uses session [key](concepts?id=key) to subscribe and set store state. If there are multiple useQuery/useMutation with a same session key executes at the same time, only one can be allowed to execute, others just wait for the state change by subscribing store.
