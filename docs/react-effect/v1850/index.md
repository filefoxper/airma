...
Both of useQuery and useMutation need a promise callback for working, the mission of promise callback is called [session](concepts?id=session).
...
The state `data` from useSession is always has a `undefined` union type. API [useLoadedSession](api?id=useloadedsession) can be helpful if the session `state.data` is not empty from initializing time.
