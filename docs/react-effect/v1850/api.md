...
* promiseCallbackOrSessionKey - It can be a promise callback or a session [key](concepts?id=key), event a session store.
...
* storeCreators - [Session keys](concepts?id=key), [model keys](/react-state/concepts?id=key) or store.
