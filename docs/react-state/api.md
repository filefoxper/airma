...
The fields of instance generated by useModel are static in render time. The fields of realtime instance are dynamic in render time.
...
* **instance** - Instance object generated by useModel API.
...
const saveUser =()=>{
  // When setTimeout start, callSetAge.
  // The instance.user.age should be a new age when time is out,
  // but it not.
  // The realtimeInstance.user.age will change when time is out.
  setTimeout(()=>{
    // instance.user.age is still not change
    instance.user.age;
    // realtimeInstance.user.age is a new one.
