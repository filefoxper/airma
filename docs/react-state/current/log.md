...
* use as-model as a core.
* add model(xxx).produce(xxx) to simulate an asynchronous support instance.
* add createStore(xxx) API.
* remove some deprecated session API: 
  * model().createStore().static(), 
  * model().createStore().with(), 
...
* make signal callback persist which is generated by useSignal. 
