...
API `model.createCacheField` helps creating a cache field for a model instance. It is helpful for using instance fields with react effect hooks.
...
const queryModel = model((condition:QueryCondition)=>{
    return {
        ...
        // Only when it is getted from model instance, and called the get method, the cache effect can be applied.
...
**Note**, the result generated by calling API `model.createCacheField` should be hang on the instance or dependent by another cache field which has been hang on instance. For the cache effect is given by instance.
