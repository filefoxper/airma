...
The API [createKey](api?id=createkey) can wrap a model function to be a unique `key` with or without a default state. The example above shows how to initialize default state with `key`. There is also a way for initializing default state in render time.
