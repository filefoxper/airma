...
* modelLike - A function accepts a state parameter, and returns an object to provide display data and action methods. It also can be a model key, created by [createKey](api?id=createkey) API or model(modelFn).createKey(). Event a store created by model(modelFn).createStore().
...
This API can help you reuse model more easier. It is drived by an outside state, and feedback a next state to change the outside state when the action method from instance calls. It has no state inside. You can refer to [guide section](guides?id=usecontrolledmodel) for detail.
...
* modelKeyOrStore - A model key, it should be a result of calling [createKey](api?id=createkey). Or a model store created by model(modelFn).createStore().
...
* modelFnOrKey - A function accepts a state parameter, and returns an object to provide display data and action methods. It also can be a model key, created by [createKey](api?id=createkey) API.
* defaultState - Optional, a default state for model initializing.
